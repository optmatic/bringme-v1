...
const getCategoryData = (category: string) => {
  const categoryMap: Record<string, { title: string; description: string }> = {
    australia: {
      title: "Australia",
      description: "In-depth coverage of Australian politics, society, and culture.",
    },
    world: {
      title: "World",
      description: "Global perspectives on international affairs and their impact on Australia.",
    },
    politics: {
      title: "Politics",
      description: "Analysis and insights on Australian political landscape and policy debates.",
    },
    feed: {
      title: "Feed",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50/30 via-lime-50/20 to-green-50/30">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <CategoryHero title={categoryData.title} description={categoryData.description} />

        {/* Render different content based on category */}
        {category === "discover" ? <DiscoverContent /> : <CategoryArticleList category={category} />}
      </main>
      <Footer />
    </div>
  )
